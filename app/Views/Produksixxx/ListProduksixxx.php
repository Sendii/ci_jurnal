<main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
      <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        <h1 class="h2">DATA Biaya Produksi SiPuri</h1>
      ...
      <div class="row">
        <div class="col">
            <div class="card">
              <div class="card-body">
              <a href="<?= base_url('biayaproduksi') ?>" class="btn btn-warning" id="tmbh">Tambah Perhitungan Biaya Produksi</a>
                    ...
                    foreach($targetcosting as $row):
                        ?>
                            <tr>
								...
								<td><?= rupiah($row->satuan_bb);?></td>
								<td><?= $row->qty_bb;?></td>
								<td><?= rupiah($row->harga_bb);?></td>
								<td><?= rupiah($row->jenis_bb);?></td>
								<td><?= rupiah($row->total_bb);?></td>
								
								<td><?= $row->nama_bop;?></td>
								<td><?= $row->qty_bop;?></td>
								<td><?= rupiah($row->satuan_bop);?></td>
								<td><?= rupiah($row->harga_bop);?></td>
								
								<td><?= $row->nama_karyawan;?></td>
								<td><?= $row->qty_karyawan;?></td>
							    <td><?= rupiah($row->satuan_karyawan);?></td>
							    <td><?= rupiah($row->harga_karyawan);?></td>
								
								<td><?= rupiah($row->biayaproduksi);?></td>
...
<script>
          function deleteConfirm(url){
              url2 = "<?= base_url('Produksig/ListProduksi') ?>"; //diisi dengan halaman ini
