...
class bukubesar extends BaseController
{
	...
	public function index(){
		...
		$akun = !isset($_GET['akun']) || $_GET['akun'] == "" ? "111" : $_GET['akun'];
		$data['list'] = $this->db->query("SELECT a.id_transaksi, a.kode_akun, a.tgl_jurnal, a.posisi_d_c, a.nominal, b.nama_akun 
			FROM jurnal a
			LEFT JOIN akun b ON a.kode_akun = b.kode_akun where a.kode_akun = ${akun} and YEAR(a.tgl_jurnal) = ${year} and month(a.tgl_jurnal) = ${month}")->getResult();
		$data['akuns'] = $this->db->table('akun')->get()->getResult();

		echo view('bukubesar/index', $data);
	}
}